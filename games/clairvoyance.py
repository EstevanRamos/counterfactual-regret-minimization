from common.clairvoyance_constants import CHECK, BET, CALL, FOLD, A, CHANCE, RESULTS_MAP
import random
...
class ClairvoyanceRootChanceGameState(GameStateBase):

    def __init__(self, actions):
        super().__init__(parent = None, to_move = CHANCE, actions = actions)
        self.children = {
            cards: ClairvoyancePlayerMoveGameState(
...
class ClairvoyancePlayerMoveGameState(GameStateBase):
    ...
    def __init__(self, parent, to_move, actions_history, cards, actions):
        ...
        self.cards = cards
        self.children = {
            a : ClairvoyancePlayerMoveGameState(
                self,
                -to_move,
